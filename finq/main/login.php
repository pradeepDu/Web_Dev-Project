...
include "../include/dbconnect.php";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["signup"])) {
            ...
            $stmt = $conn->prepare("SELECT user_name FROM login WHERE user_name = ?");
            if (!$stmt) {
            ...
            $sql = $conn->prepare("INSERT INTO login (user_name, email, password,userfullname) VALUES (?, ?, ?,?)");
            if (!$sql) {
                throw new Exception("Prepare failed: " . $conn->error);
            }

            // Bind parameters to the prepared statement
            $sql->bind_param("ssss", $user_name, $email, $hashed_password,$fullname);
            ...
            if ($result->num_rows == 1) {
                ...
                if (password_verify($signin_password, $hashed_password)) {
                    ...
                    $_SESSION['username']=$row['user_name'];
                     header("Location: ../userquery/query.php");
                     exit();
                } else {
